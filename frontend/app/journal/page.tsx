...
import { Sentiment, RiskAssessment, AIAnalysis, Journal, Message } from '@zenly/shared';
...
export default function JournalPage() {
  ...
  const filterJournals = () => {
    ...
    // Sentiment filter
    if (filterType !== 'all') {
      filtered = filtered.filter((j) => j.aiAnalysis?.sentiment?.label === filterType);
    }
  ...
  const saveAndReflect = async () => {
    ...
    try {
      ...
      if (data.success && (data.journal || data.data?.journal)) {
        ...
        try {
          localStorage.setItem('zenly_journal_updated', Date.now().toString());
        } catch {}
  ...
  const startReflection = async (journalId: string) => {
    try {
      ...
      const es = new EventSource(
        `http://localhost:5001/journals/${journalId}/analyze-stream?token=${encodeURIComponent(token)}`
      );
        ...
        try {
          ...
          const pct =
            data.progress && data.progress <= 1
              ...
              : Math.round((data.progress || 0) as number);
      ...
      const onComplete = async (e: MessageEvent) => {
        try {
          ...
          setJournals((prev) =>
            prev.map((j) =>
              j._id === journalId
                ? ({ ...j, status: 'analyzed', aiAnalysis: data.analysis } as any)
                : j
            )
          );
          const updated = ((prev) => prev.find((j) => j._id === journalId))(
            ((j) => j)([...journals])
          );
          if (updated)
            setSelectedJournal({
  ...
  const loadJournalAndStartReflection = async (journalId: string, journalContent: string) => {
    try {
      ...
      if (existingMessages.length > 0) {
        console.log(
          '[Journal AI] Messages already exist (',
          existingMessages.length,
          '), opening panel without sending'
        );
        setIsAIPanelOpen(true);
        return;
      }
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m._id === tempUserMessage._id ? { ...m, status: 'error' as const } : m))
      );
                ...
                {isLoadingJournals ? (
                  ...
                  filteredJournals.map((journal) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-700 line-clamp-2 mb-2">{journal.content}</p>
          ...
          <div className="flex-1 flex flex-col">
            <Card className="flex-1 flex flex-col">
              <CardHeader>
                ...
                <CardDescription>
                  Express yourself freely. Click "Reflect with AI" to get insights and talk about
                  it.
                </CardDescription>
              </CardHeader>
      ...
      <Sheet open={isAIPanelOpen} onOpenChange={setIsAIPanelOpen}>
        <SheetContent side="right" className="w-[500px] sm:w-[600px] flex flex-col p-0">
          <SheetHeader className="p-6 pb-4">
            ...
            <SheetDescription>Talk with AI about your journal entry</SheetDescription>
          </SheetHeader>
