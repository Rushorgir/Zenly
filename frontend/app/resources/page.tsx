...
import { Resource, FeaturedResources } from '@zenly/shared';
...
export default function ResourcesPage() {
  ...
  useEffect(() => {
    ...
    socket.on('resource:likeUpdate', ({ resourceId, helpfulCount }) => {
      ...
      setSearchResults((prev) =>
        prev.map((r) => (r._id === resourceId ? { ...r, helpfulCount } : r))
      );
    });
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/resources/search?query=${encodeURIComponent(searchQuery)}`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const trackView = async (id: string) => {
    try {
      ...
      const token =
        typeof window !== 'undefined' ? localStorage.getItem('zenly_access_token') : null;
  ...
  const renderVideoCard = (resource: Resource) => {
    ...
    const parsed = (() => {
      if (resource.embedData && (resource.embedData.embedId || resource.embedData.platform))
      ...
      >
        <div className="relative aspect-video bg-gray-900">
          {platform === 'youtube' && embedId ? (
            ...
            <img
              src={parsed.thumbnailUrl || `https://img.youtube.com/vi/${embedId}/hqdefault.jpg`}
              alt={resource.title}
      ...
      >
        <div className="relative aspect-video bg-gradient-to-br from-green-500/20 to-emerald-500/20 overflow-hidden">
          {resource.embedData?.thumbnailUrl ? (
            <img
              src={resource.embedData.thumbnailUrl}
              alt={resource.title}
              className="w-full h-full object-cover"
            />
                  ...
                  {filteredResults.length === 0 ? (
                    <Card>
                      <CardContent className="py-12 text-center text-muted-foreground">
                        ...
                        <p>
                          No {selectedType === 'all' ? '' : selectedType} resources found matching "
                          {searchQuery}"
                        </p>
